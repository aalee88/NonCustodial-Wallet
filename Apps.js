...
const App = () => {
  ...
  async function network(event) {
    ...
    try {
      if (x !== "homestead" && x !== "mainnet" && x !== "matic") {
        ...
        console.log(wallet);
        setCurrentNetwork(
          "Polygon (" + x.charAt(0).toUpperCase() + x.slice(1) + ") Network"
        );
        console.log("Changing network to " + x + " Network");
        setNetworkMessage("Successful!");
  ...
  async function addAccount(event) {
    ...
    try {
      ...
      var file = new Blob([jsons], { type: "string/*" });
      element.href = URL.createObjectURL(file);
      element.download = event.target.walletname.value + ".json";
      element.click();
      setAddMessage(
        "JSON Wallet has been saved to browser and downloaded to device"
      );
      event.target.Password.value = "";
      event.target.walletname.value = "";
    } catch (err) {
  ...
  async function open1() {
    try {
      ...
      console.log(x);
      setCreateMessage(
        "Wallet successfully created! Proceed to your Wallet below"
      );
      r.style.setProperty("--display", "none");
      r.style.setProperty("--displays", "none");
    } catch (err) {
  ...
  async function getPersonals(event) {
    ...
    if (fromPv === false) {
      ...
      const x = await walletMnemonic.mnemonic;
      setMnemonic(
        `"NO MNEMONIC DERIVED! {Note that a mnemonic phrase cannot be derived from wallets imported using a private key}"`
      );
      setPrivateKey(await walletMnemonic.privateKey);
      if (view === "View Details") {
        r.style.setProperty("--displayss", "block");
  ...
  async function getTx(event) {
    ...
    const ethTxFee =
      ((ethers.utils.parseUnits(`${Math.ceil(f)}`, "gwei") /
        1000000000000000000) *
        21000) /
      1000000000;
    ...
    try {
      ...
      if (ti.includes(event.target.wallet.value) === true) {
        let x = window.confirm(
          'Are you sure you want to delete the wallet with ID "' +
            event.target.wallet.value +
            '"? THIS ACTION IS IRREVESIBLE ONCE CONFIRMED'
        );
        if (x === true) {
          localStorage.removeItem("wallet-" + event.target.wallet.value);
          setDel(
            '"' + event.target.wallet.value + '" wallet has been deleted!'
          );
          event.target.wallet.value = "";
        } else {
  ...
  function clear(event) {
    ...
    try {
      ...
      if (th.length !== 0) {
        let x = window.confirm(
          "Are you sure you want to CLEAR ALL WALLETS SAVED TO THIS BROWSER? THIS ACTION IS IRREVESIBLE ONCE CONFIRMED"
        );
        if (x === true) {
          localStorage.clear();
  ...
  return (
    <div className="App">
      ...
      <div className="onboard">
        ...
        <form onSubmit={open}>
          <input placeholder="Mnmeonic" id="mnemonic" type="text" />
          <p></p>
          <p></p>
        ...
        <form onSubmit={open2}>
          <input placeholder="Private Keys" id="privatekey" type="text" />
          <p></p>
          <p></p>
          <button type={"submit"}> Import Wallet </button>
        </form>
        ...
        <h4>
          {" "}
          This is for quick login in from the JSON file stored in your browser{" "}
        </h4>
        <h5>
          <p style={{ color: "red", marginTop: "1em" }}>{warn}</p>
        </h5>
        <p></p>
        {warn ? (
          <button
        ...
        <button onClick={listAccounts}>
          {" "}
          {listAccount} all saved wallets{" "}
        </button>
        <p></p>
        <li className="list" style={{ marginBottom: "3em" }}>
          {" "}
          {listItems}{" "}
        </li>
        ...
        {wallet ? (
          <div className="button">
            <button class="button1" onClick={show}>
              {" "}
              WALLET{" "}
            </button>
            <button class="button1" onClick={shows}>
              {" "}
              SETTINGS{" "}
            </button>
          </div>
      ...
      <div className="wallet">
        ...
        <h6>
          {" "}
          *Supported Main-Network Names: Mainnet, Matic. <br /> *Supported
          Test-Network Names: Kovan, Ropsten, Goerli, Rinkeby.
        </h6>
        <form onSubmit={network}>
          <input placeholder="Network name" id="network" type="text" />
          <p></p>
          <p></p>
        ...
        <div className="transaction">
          ...
          <form onSubmit={getTx}>
            <input
              placeholder="Address or ENS ('.eth') name"
              id="recipient"
              type="text"
            />
            <p></p>
          ...
          <h6>
            {" "}
            *To view {address}'s tokens and balances on other chains/networks
            apart <br /> from {currentNetwork} switch over to that chain and
            click the button below.
            <br /> It will redirect you to that chain's explorer/scan.
          </h6>
        </div>
      </div>
      ...
      <div className="settings">
        ...
        <div className="encrypt">
          ...
          <h5>
            {" "}
            This encrypts your wallet as a JSON file with a password of your
            choice,
            <br /> stores it on your browser as the 'Wallet-Name' you chose and
            downloads the file <br />
            into your device's storage as ('Wallet-Name' you chose).JSON
            <p style={{ color: "red", marginTop: "2.5em" }}>{warn1}</p>
          </h5>
          {warn1 ? (
            <button
              className="button2"
          ...
          <form onSubmit={addAccount}>
            <input placeholder="Wallet Name" id="walletname" type="text" />
            <p></p>
            <p></p>
            <input placeholder="Password" id="Password" type="text" />
            <p></p>
        ...
        <div className="personal">
          ...
          <div className="personals">
            <p>
              {" "}
              Your mnemonic phrase: <p></p> {mnemonic}{" "}
            </p>

            <p>
              {" "}
              Your Private Keys: <p></p> {privateKey}{" "}
            </p>
          </div>
          ...
          <div className="delete">
            <h5>
              {" "}
              Input the name of the wallet you wish to delete from your
              browser's storage.
            </h5>

            <form onSubmit={deleteWallet}>
              <input placeholder="Wallet Name" id="wallet" type="text" />
              <p></p>
              <p></p>
            ...
            <h3>
              {" "}
              CLEAR ALL ACCOUNT DATA OF BROWSER WALLET FROM YOUR BROWSER{" "}
            </h3>
